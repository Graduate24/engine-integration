...
import codeanalysis.saengine.entity.DetectedResult;
import codeanalysis.saengine.entity.PathUnit;
import codeanalysis.saengine.entity.SmapInfo;
...
public class PathUtil {
    ...
    public static List<String> pathStm(ResultSourceInfo source) {
        return Arrays.stream(source.getPath()).map(String::valueOf).collect(Collectors.toList());
    }
    ...
    public static String locateSourceFile(String projectDir, String javaClass) {
        ...
        List<String> sourceFile = FileUtility.filterFile(projectDir, new String[]{"**/" + path});
        if (sourceFile.isEmpty()) {
            return null;
        }
        System.out.println("find source:" + sourceFile.get(0));
