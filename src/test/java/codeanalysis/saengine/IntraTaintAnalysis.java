...
public class IntraTaintAnalysis {
    ...
    public IntraTaintAnalysis(SootMethod mtd, InterTaintAnalysis interProcAna) {
        ...
        //TODO
        List<PDGNode> pdgnodes = pdg.getDependents(pdg.GetStartNode()); //pdg node list
    ...
    private void kill(LinkedHashSet<String> entrySet, LinkedHashSet<String> exitSet, Unit stmt) {
        ...
        if (!isInvokedByt_edExpr) {
            if (stmt instanceof JAssignStmt) {
                ...
                String lhsv = assignS.getLeftOpBox().getValue().toString();
                boolean t_Chk = false;
                if (!lhsv.contains("[")) {
                    Value rightv = assignS.getRightOpBox().getValue();
                    if (rightv instanceof JStaticInvokeExpr) {
                        ...
                        if (assignS.getRightOpBox().getValue() instanceof Constant) {
                            exitSet.remove(lhsv);
                            interAna.KillMapCreation(lhsv, assignS.getRightOpBox().getValue().toString(), currentBlock.getBody().getMethod());
                        } else {
                            Iterator itsUse = assignS.getUseBoxes().iterator();
                            Value v = assignS.getRightOpBox().getValue();
    ...
    private void gen(LinkedHashSet<String> entrySet, LinkedHashSet<String> exitSet, Unit stmt) {
        if (stmt instanceof JIdentityStmt) {
            ...
            Value lhs = idStmt.getLeftOpBox().getValue();
            Value rhs = idStmt.getRightOpBox().getValue();
